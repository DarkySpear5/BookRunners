...
export function UpdatePrompt(): React.JSX.Element | null {
  ...
  return (
    <Modal title={t('update_available_title')} onClose={() => setDismissed(true)} width="max-w-sm">
      {phase === 'available' && (
        <>
          <p className="text-sm text-text">{t('update_available_msg', { version: info.version })}</p>
      ...
      {phase === 'downloading' && (
        <>
          <p className="mb-2 text-sm text-text">{t('downloading_update_msg', { percent: Math.round(percent) })}</p>
      ...
      {phase === 'downloaded' && (
        <>
          <p className="text-sm text-text">{t('update_ready_msg')}</p>
          <div className="mt-4 flex justify-end gap-2">
            <button
              onClick={() => setDismissed(true)}
              className="rounded bg-card px-3 py-1.5 text-sm text-text hover:bg-card/70"
