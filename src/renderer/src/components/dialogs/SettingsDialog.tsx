  ...
  return (
    <label className="flex items-center justify-between rounded bg-card px-3 py-2.5 text-sm text-text">
      ...
      <input
        ...
        className="h-4 w-4 accent-[var(--br-accent)]"
      />
    </label>
  )
}
...
function UiTab({ settings }: { settings: { fontFamily: string; fontScale: number; coverSize: number } }): React.JSX.Element {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div>
        <label className="mb-1 block text-xs text-subtext">{t('label_cover_size')}</label>
        <select
          value={settings.coverSize}
          onChange={(e) => void updateSettings({ coverSize: parseInt(e.target.value, 10) })}
          className="w-full rounded bg-card px-2.5 py-1.5 text-sm text-text outline-none"
        >
          {Object.entries(COVER_SIZE_OPTIONS).map(([label, value]) => (
