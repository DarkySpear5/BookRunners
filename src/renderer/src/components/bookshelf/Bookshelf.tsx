...
export function Bookshelf(): React.JSX.Element {
  ...
  // Delete key deletes the selected book (still confirms) — matches v1's
  // <Delete> binding on the bookshelf. Skipped while typing anywhere
  // (rename field, notes, add-book input, etc.) so it never eats a
  ...
  return (
    <div className="relative flex h-full shrink-0 flex-col bg-panel" style={{ width }}>
      <div className="px-4 pt-4 pb-1 text-xs font-medium tracking-wide text-subtext">{t('label_bookshelf')}</div>
      ...
      <div className="flex flex-col gap-1.5 px-2.5 pb-2">
        ...
        >
          <option value="name">{t('sort_title_az')}</option>
          <option value="author">{t('sort_author_az')}</option>
          <option value="last_read">{t('sort_last_read')}</option>
      ...
      >
        {sorted.length === 0 && (
          <div className="px-3 py-6 text-center text-xs text-subtext">
            {settings?.genreFilter && settings.genreFilter !== 'All'
              ? t('empty_genre_filter', { genre: t(settings.genreFilter, { ns: 'genres' }) })
              : t('empty_no_books')}
          </div>
        )}
        {sorted.map((p) => {
        ...
        <ContextMenu
          ...
          items={
            contextMenu.target
              ...
              : [
                  { label: t('menu_add_book'), onClick: () => setAdding(true) },
                  { label: t('ctx_import'), onClick: () => void handleImport() }
                ]
          }
        />
        ...
        {adding ? (
          <div className="flex gap-1.5">
            <input
              ...
              placeholder={t('dlg_add_book_prompt')}
