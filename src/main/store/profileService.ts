...
export const profileService = {
  ...
  async duplicate(name: string): Promise<Profile> {
    ...
    if (original.iconFile) {
      const oldPath = join(paths.coversDir(), original.iconFile)
  ...
  async setAuthor(name: string, author: string): Promise<Profile> {
    const profile = requireProfile(name)
  ...
  async setStatus(name: string, status: Status): Promise<Profile> {
    if (status !== 'reading' && timerEngine.isRunning(name)) {
    ...
    if (status === 'reading') {
      ...
      // Only "finished" gets a meaningful Time Finished snapshot in the Library
