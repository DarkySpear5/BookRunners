 ...
 */
const StatusSchema = z.enum(['reading', 'finished', 'dropped', 'on_hold']).catch('reading')
...
const ProfileSchema = z
  .object({
    ...
    lastRead: z.number().nullable().catch(null),
...
const SettingsSchema = z.object({
  ...
  coverSize: z.number().catch(36),
  theme: ThemeNameSchema,
  customColors: ThemeColorsSchema,
  fontFamily: z.string().catch('Georgia'),
  fontScale: z.number().catch(1.0),
  sortMode: z.enum(['name', 'author', 'last_read', 'rating', 'genre']).catch('name'),
  genreFilter: z.string().catch('All'),
  statusFilter: z
    .union([z.literal('All'), z.enum(['reading', 'finished', 'dropped', 'on_hold'])])
    .catch('All'),
