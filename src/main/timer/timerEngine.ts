...
/**
 * Any number of books can run concurrently — a profile is "running" iff it's
 * a key in activeTimers, mapping name -> the Date.now() ms timestamp Read was
 * last pressed (or last checkpointed). Mirrors v1's self.tick_start /
 * _toggle_play()/_checkpoint() exactly, just in ms instead of float seconds
 * internally (converted to seconds wherever it touches `profile.seconds`,
 * which stays in seconds to match the persisted format).
 */
class TimerEngine {
  ...
  start(name: string): void {
    ...
    if (profile.status === 'dropped' || profile.status === 'on_hold') {
      // Pressing Read is the natural "I'm actually reading this again"
      // signal — Finished survives a re-read, but a stale Dropped/On Hold
      // label doesn't (matches v1's _start_current design, extended for the
      // 4-state model).
