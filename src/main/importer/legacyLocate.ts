...
const INNO_UNINSTALL_KEY =
  'HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{6010DD4A-12DB-42BC-835F-8D1FBB725EA3}_is1'
...
async function findViaRegistry(): Promise<string | null> {
  try {
    ...
    const dataFile = join(dir, 'bookrunners_data.json')
...
function findViaDefaultPath(): string | null {
  const dataFile = join(os.homedir(), 'AppData', 'Local', 'Programs', 'BookRunners', 'bookrunners_data.json')
  return existsSync(dataFile) ? dataFile : null
}
...
async function findViaScan(): Promise<string | null> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const dataFile = join(programsDir, entry.name, 'bookrunners_data.json')
