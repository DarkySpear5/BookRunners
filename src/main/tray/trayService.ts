...
const GREEN_RGBA: RgbaColor = [143, 174, 122, 255]
const RED_RGBA: RgbaColor = [217, 138, 122, 255]
const BORDER_RGBA: RgbaColor = [74, 52, 40, 255]
...
/**
 * Mirrors v1's pystray icon exactly: a colored status dot (sage green if
 * anything is being read, rose red otherwise), a 3-item menu (Show / Read-Pause
 * the selected book / Quit), and a tooltip naming the selected book + its
 * state. Callers explicitly call refresh() after anything that changes
 * running or selected state (see ipc/timer.ipc.ts, ipc/profiles.ipc.ts)
 * rather than this service polling.
 */
class TrayService {
